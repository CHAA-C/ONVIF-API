...
function startProbe(START_IP_ADDRESS) {
    MULTICAST_ADDRESS = START_IP_ADDRESS || MULTICAST_ADDRESS;
    if (!START_IP_ADDRESS) {
        START_IP_ADDRESS = MULTICAST_ADDRESS;
    }
    if (START_IP_ADDRESS) {
        var START_IP_PARTS = START_IP_ADDRESS.split('.');
        if (START_IP_PARTS.length !== 4) {
            return Promise.reject(new Error('Invalid IP Address Format...'));
        }
        var BASE_IP_ADDRESS = START_IP_PARTS.slice(0, 3).join('.');
        var START_RANGE = parseInt(START_IP_PARTS[3]);
        var END_RANGE = 254;
    }
    return new Promise(function (resolve, reject) {
        ...
        udp.bind(function () {
            ...
            for (var i = START_RANGE; i <= END_RANGE; i++) {
                var IP = BASE_IP_ADDRESS + '.' + i;
                sendProbe(IP).catch(function (e) { reject(e); });
            }
    ...
    return new Promise(function (resolve, reject) {
        ...
        var send = function () {
            ...
            if (soap) {
                ...
                udp.send(buf, 0, buf.length, PORT, IP_ADDRESS, function () {
                    discoveryIntervalTimer = setTimeout(function () {
                        send();
                    }, DISCOVERY_INTERVAL);
                });
